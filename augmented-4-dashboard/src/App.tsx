...
const PIN_COOKIE_KEY = 'dashboard_auth';
const PIN_CODE = '060626';
const PIN_EXPIRY_HOURS = 12;
...
const App = () => {
  ...
  const [isAuthenticated, setIsAuthenticated] = useState(() => {
    const authCookie = Cookies.get(PIN_COOKIE_KEY);
    return authCookie === 'authenticated';
  });

  // Show modal if not authenticated
  const [showPinModal, setShowPinModal] = useState(!isAuthenticated);
  ...
  const handlePinChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (value.length === 6) {
      if (value === PIN_CODE) {
        ...
        Cookies.set(PIN_COOKIE_KEY, 'authenticated', { 
          expires: PIN_EXPIRY_HOURS / 24,  // Convert hours to days
  ...
  // Handle manual logout (if you want to add this feature)
  const handleLogout = () => {
    Cookies.remove(PIN_COOKIE_KEY);
    ...
    <div className="relative">
      {showPinModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-gray-900 rounded-2xl shadow-2xl p-8 flex flex-col items-center border-2 border-blue-700 min-w-[320px]">
            ...
            <input
              ...
              className="w-full px-4 py-2 bg-gray-800 border-2 border-gray-700 rounded-lg text-center text-2xl tracking-widest text-white mb-4"
              placeholder="• • • • • •"
              maxLength={6}
              inputMode="numeric"
              pattern="[0-9]*"
            ...
            <p className="text-gray-400 text-xs mt-4">Session expires in {PIN_EXPIRY_HOURS} hours</p>
          </div>
        </div>
      )}
      <div className={`min-h-screen bg-gray-900 text-white ${showPinModal ? 'pointer-events-none' : ''}`}>
        ...
        <header className="w-full bg-black py-6 px-6 flex items-center shadow-lg z-50 relative overflow-hidden">
          ...
          {/* Title - hidden on mobile, visible from md breakpoint */}
          <h1 className="hidden md:block text-3xl font-bold text-white absolute left-1/2 transform -translate-x-1/2 -translate-y-1/2 drop-shadow-[0_0_10px_white] z-10 whitespace-nowrap">
        ...
        <div className="container mx-auto px-4 py-8">
              ...
              <div className="p-6 space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
                  ...
                  <div className="bg-gray-900 p-4 rounded-lg border border-gray-700">
                    ...
                    <div className="text-lg font-bold text-green-400">12 june 2025</div>
          ...
          <div className="mt-8 text-center text-gray-500 text-sm">
            ...
            <p>Shareholders Agreement: 30 May 2025 | Constitution: 12 june 2025</p>
          </div>
        </div>
      </div>
    </div>
